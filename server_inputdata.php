...
$dbname = "dom_dbs";  // Change to your database name
...
$ng_qty = isset($_POST['ngqty']) ? intval($_POST['ngqty']) : 0;
...
$sql = "SELECT  plan_tol_hr, cntPerhr, cntTolhr FROM data_output WHERE id = ?";
...
if ($result->num_rows > 0) {
    ...
    $sql = "UPDATE data_output SET cntPerhr=?, cntTolhr=?, achieved=? WHERE id=?";
    $stmt = $conn->prepare($sql);
